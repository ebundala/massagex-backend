...
export class GoogleMapGeolocationResolver {
  private _key: string;
  constructor(
    private readonly service: GoogleMapService,
    private readonly configService: ConfigService,
  ) {
    this._key = this.configService.get<string>(
      AppConfigurationKeys.GOOGLE_MAPS_API_KEY,
    );
  }
  ...
  async reverseGeocode(
    @Parent() _,
    @Args('data') args: ReverseGeocodeInput,
    @Context() ctx: TenantContext,
    @Info() info,
  ) {
    //todo: geocode here
  ...
  async route(
    @Parent() _,
    @Args('data') args: RouteQueryInput,
    @Context() ctx: TenantContext,
    @Info() info,
  ) {
    //todo: route here
  ...
  async placesAutocomplete(
    @Parent() _,
    @Args('data') args: PlaceAutocompleteInput,
    @Context() ctx: TenantContext,
    @Info() info,
  ) {
    try {
