...
export class PaymentService {
  ...
  async paybill(data: PaybillRequest, ctx: TenantContext, info) {
    ...
    const methodItem = await prisma.paymentMethod.findUnique({
      where: { id: method },
    });
    if (methodItem == null) {
    ...
    if (methodItem.code == SelcomUtilityCode.VMCASHIN) {
      ...
      const select = prisma
        .getSelection(info)
        .valueOf('data', 'Transaction', { select: {} });
      let tx;
            ...
            mpesaPayment: {
              update: {
                ...
                output_ThirdPartyConversationID:
                  result.output_ThirdPartyConversationID,
      ...
      throw new HttpException(
        { status: false, message: 'Payment method not supported' },
        400,
      );
    }
  }
