...
} from 'src/app-schemas/subscriptions/subscription.service';
import {
  Business,
  LocationCreateInput,
  User,
...
import { buildGeoQuery } from './geo-query-builder';
...
export class BusinessLogicService {
  ...
  @BlocAttach(
    'updateOneUser.input.data.reviewed.create.attachments.create.path',
  )
  async uploadReviewAttachments(
    v: BusinessRequest<TenantContext>,
    next: (arg0: BusinessRequest<any>) => any,
  ) {
    const { args, context } = v;
    ...
    for (let i = 0; i < reviews.length; i++) {
      ...
      for (let j = 0; j < attachments.length; j++) {
        const task = new Promise(async (resolve, reject) => {
          ...
          const file2 = await context.prisma.attachment.create({
            data: { ...file },
          });
          if (file2 && file2.id) {
            let connect =
              v.args.data.reviewed.create[i].attachments.connect ?? [];
  ...
  @BlocAttach(
    'updateOneUser.input.data.reviewed.update.data.attachments.create.path',
  )
  async uploadReviewAttachments2(
    v: BusinessRequest<TenantContext>,
    next: (arg0: BusinessRequest<any>) => any,
  ) {
    const { args, context } = v;
    ...
    for (let i = 0; i < reviews.length; i++) {
      ...
      for (let j = 0; j < attachments.length; j++) {
        const task = new Promise(async (resolve, reject) => {
          ...
          if (file2 && file2.id) {
            let connect =
              v.args.data.reviewed.update[i].data.attachments.connect ?? [];
            connect[j] = { id: file2.id };
  ...
  @BlocAttach(
    'updateOneUser.input.data.comments.create.attachments.create.path',
  )
    ...
    for (let i = 0; i < commentsList.length; i++) {
      ...
      for (let j = 0; j < attachments.length; j++) {
        const task = new Promise(async (resolve, reject) => {
          ...
          if (file2 && file2.id) {
            let connect =
              v.args.data.comments.create[i].attachments.connect ?? [];
            connect[j] = { id: file2.id };
  ...
  @BlocAttach(
    'updateOneUser.input.data.comments.update.data.attachments.create.path',
  )
  async uploadCommentAttachments2(
    v: BusinessRequest<TenantContext>,
    next: (arg0: BusinessRequest<any>) => any,
  ) {
    const { args, context } = v;
    ...
    for (let i = 0; i < commentsList.length; i++) {
      ...
      for (let j = 0; j < attachments.length; j++) {
        const task = new Promise(async (resolve, reject) => {
          ...
          if (file2 && file2.id) {
            let connect =
              v.args.data.comments.update[i].data.attachments.connect ?? [];
            connect[j] = { id: file2.id };
  ...
  @BlocAttach(
    'updateOneUser.input.data.businessProfile.update.cover.create.path',
  )
  async uploadBusinessCoverAttachments(
    v: BusinessRequest<TenantContext>,
    next: (arg0: BusinessRequest<any>) => any,
  ) {
    const { args, context } = v;
  ...
  @BlocAttach(
    'updateOneUser.input.data.businessProfile.upsert.create.cover.create.path',
  )
  async upsertBusinessCoverAttachments(
    v: BusinessRequest<TenantContext>,
    next: (arg0: BusinessRequest<any>) => any,
  ) {
    const { args, context } = v;
  ...
  @BlocAttach(
    'updateOneUser.input.data.businessProfile.update.attachments.create.path',
  )
  async uploadGalleryAttachments(
    v: BusinessRequest<TenantContext>,
    next: (arg0: BusinessRequest<any>) => any,
  ) {
    const { args, context } = v;
    ...
    for (let j = 0; j < attachments.length; j++) {
      const task = new Promise(async (resolve, reject) => {
        ...
        if (file2 && file2.id) {
          let connect =
            v.args.data.businessProfile.update.attachments.connect ?? [];
          connect[j] = { id: file2.id };
  ...
  @BlocAttach(
    'updateOneUser.input.data.businessProfile.update.services.create.image.create.path',
  )
  async uploadServicetAttachment(
    v: BusinessRequest<TenantContext>,
    next: (arg0: BusinessRequest<any>) => any,
  ) {
    const { args, context } = v;
    ...
    for (let i = 0; i < services.length; i++) {
      const task = new Promise(async (resolve, reject) => {
        ...
        if (file2 && file2.id) {
          businessProfile.update.services.create[i].image.connect = {
            id: file2.id,
          };
  ...
  @BlocAttach(
    'updateOneUser.input.data.businessProfile.update.services.update.data.image.create.path',
  )
  async uploadServiceAttachment2(
    v: BusinessRequest<TenantContext>,
    next: (arg0: BusinessRequest<any>) => any,
  ) {
    const { args, context } = v;
    ...
    for (let i = 0; i < services.length; i++) {
      ...
      const task = new Promise(async (resolve, reject) => {
        ...
        if (file2 && file2.id) {
          businessProfile.update.services.update[i].data.image.connect = {
            id: file2.id,
          };
          businessProfile.update.services.update[i].data.image.create =
            undefined;
  ...
  @BlocAttach(
    'updateOneUser.input.data.businessProfile.update.orders.update.data.receipt.create.path',
  )
  async uploadOrderReceiptAttachment(
    v: BusinessRequest<TenantContext>,
    next: (arg0: BusinessRequest<any>) => any,
  ) {
    const { args, context } = v;
    ...
    for (let i = 0; i < orders.length; i++) {
      ...
      const task = new Promise(async (resolve, reject) => {
        ...
        if (file2 && file2.id) {
          businessProfile.update.orders.update[i].data.receipt.connect = {
            id: file2.id,
          };
          businessProfile.update.orders.update[i].data.receipt.create =
            undefined;
  ...
  async linkOrderAndBusiness(
    ...
    for (let i = 0; i < orders.length; i++) {
      ...
      const task = new Promise(async (resolve, reject) => {
        const business = await context.prisma.service.findUnique({
          where: { id: order.service.connect.id },
          select: { id: true, businessId: true },
        });
        if (business && business.businessId) {
          data.ordered.create[i].business = {
            connect: { id: business.businessId },
          };
  ...
  @BlocAttach(
    'updateOneHelp.input.data.steps.update.data.attachments.create.path',
  )
  async uploadHelpAttachment3(
    v: BusinessRequest<TenantContext>,
    next: (arg0: BusinessRequest<any>) => any,
  ) {
    const { args, context } = v;
    ...
    if (loc && loc.id) {
      ...
      if (affected) {
        ...
        if (
          ...
          user.businessProfile.mode == BusinessMode.MOBILE_MODE
        ) {
  ...
  async updateUserLocation(v: BusinessRequest<TenantContext>, next) {
    ...
    let locationInput;
    if (
      user1.businessProfile.mode == BusinessMode.OFFICE_MODE &&
    ...
    if (user.location && user.location.id) {
      const affected = await this.createGeomFromLocation(
        user.location,
        context,
      );
      if (affected) {
  ...
  @BlocAttach(
    'updateOneUser.input.data.businessProfile.create.location.create.lat',
  )
  async createBusinessLocation(v: BusinessRequest<TenantContext>, next) {
    ...
    const { name, lat, lon }: LocationCreateInput =
      businessProfile.create.location.create;
  ...
  @BlocAttach(
    'updateOneUser.input.data.businessProfile.update.location.update.lat.set',
  )
  ...
  @BlocAttach(
    'updateOneUser.input.data.businessProfile.upsert.create.location.create.lat',
  )
  async upsertBusinessLocation(v: BusinessRequest<TenantContext>, next) {
    ...
    const { businessProfile, ...others } = data;
    const { name, lat, lon, heading } =
      businessProfile.upsert.create.location.create;
    ...
    if (location && location.id) {
      ...
      if (affected) {
        businessProfile.upsert.update.location = {
          connect: { id: location.id },
        };
        businessProfile.upsert.create.location = {
          connect: { id: location.id },
        };
      }
    }
  ...
  async oneEmailPerAccount(v: BusinessRequest<TenantContext>) {
    ...
    const facts = await prisma.user.findUnique({
      ...
      await prisma.order
        ...
        .then((orders) => {
          return Promise.allSettled(
            orders.map((v) => publish(LOCATION_CHANGED_CHANNEL, { id: v.id })),
          );
        });
    }
    // Orders updates
  ...
  @BlocFieldResolver(
    ...
    function (
      ...
      ...args: [any, any, TenantContext, any]
    ) {
      ...
      return new DataLoader(
        async function (keys) {
          ...
          if (ctx.auth?.uid) {
            ...
            let location;
            const cachedLoc = await ctx.redisCache.get(
              `location/${ctx.auth?.uid}`,
            );
            if (cachedLoc) {
        ...
        async function (keys) {
          ...
          (
            (await ctx.prisma.order.groupBy({
              ...
              where: {
                orderStatus: {
                  in: [OrderStatus.ACCEPTED, OrderStatus.PROCESSED],
                },
                business: {
                  id: {
                    in: keys,
  ...
  @BlocFieldResolver(
    'User',
    'lastSeen',
    function (this: BusinessLogicService, ...args) {
      return new DataLoader(
        async function (keys) {
          const [parent, _, ctx, info] = args;
      ...
      return new DataLoader(
        async function (keys) {
          ...
          (
            ...
            joined.set(user.id, now - Date.parse(user.createdAt)),
          );
