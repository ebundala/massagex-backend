    ...
    @Query((returns)=>CommentResponse)
    findFirstComment(@Parent() parent, @Args() args, @Context() ctx: TenantContext, @Info() info):Promise<CommentResponse|any>{
      const select = ctx.prisma.getSelection(info).valueOf('data', 'Comment', { select: {  } });
      return ctx.prisma.comment.findFirst({...args,...select})
      .then((data)=>({
        status:true,
        data,
        message:'ok'
