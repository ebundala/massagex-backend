...
export class MpesaTzService {
  ...
  constructor(
    ...
    const baseUrl = this.config.get<string>(
      AppConfigurationKeys.MPESA_BASE_URL,
    );
    this.SESSION_TTL = this.config.get<number>(
      AppConfigurationKeys.MPESA_SESSION_TTL,
    );
    ...
    this.MPESA_ORG_SHORTCODE = this.config.get<string>(
      AppConfigurationKeys.MPESA_ORG_SHORTCODE,
    );
  ...
  private initKey() {
    ...
    this.KEY = new NodeRsa(publicKey, 'pkcs8-public-pem', {
  ...
  async generateToken() {
    ...
    try {
      const result = await this.httpService.axiosRef.get<SessionResponse>(
        this.links.getToken,
        {
          ...HTTP_CLIENT_CONFIG,
          headers: {
  ...
  async paybill(body: PaybillDto): Promise<PaymentResponse> {
    ...
    try {
      ...
      const result = await this.httpService.axiosRef.post<PaymentResponse>(
        this.links.payment,
        body,
        {
          ...HTTP_CLIENT_CONFIG,
